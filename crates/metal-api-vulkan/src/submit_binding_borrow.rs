...
/// the mechanism removes a copy the submission made for itself (the sixth
/// knife's A/B read `pool` −115…−174 µs and `submit_release_bindings`
/// −41…−52 µs per submission, both far outside the spread between two
/// identical control arms), and the off arm stays reachable as the control.
