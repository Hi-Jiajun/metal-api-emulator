...
class ShippedSuitePlanTests(unittest.TestCase):
    ...
    def test_shipped_plans_are_pinned(self):
        ...
        # v37 carries the declared-superset vertex interface
        # (`research/docs/23` §3.3, E-TX11), whose translated render case and
        # declaring pass `test_suite_v37.py` pins, so it joins the list too.
        self.assertEqual(len(paths), len(PINNED_PLANS) + 36)
        observed = {}
        for path in paths:
